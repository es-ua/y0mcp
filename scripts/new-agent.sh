...
<dict>
  ...
  <array>
    ...
    <string>-c</string>
    <string>source ${ENV_FILE} && cd ${WORKSPACE_PATH} && ${CLAUDE_BIN} --dangerously-load-development-channels plugin:y0slack@y0mcp</string>
  </array>
...
ExecStart=$(which claude) --dangerously-load-development-channels plugin:y0slack@y0mcp
Restart=on-failure
RestartSec=30
